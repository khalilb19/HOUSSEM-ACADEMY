...
class HoussemAcademySupabaseUser extends BaseAuthUser {
  HoussemAcademySupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> houssemAcademySupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = HoussemAcademySupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
